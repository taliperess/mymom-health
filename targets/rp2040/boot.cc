...
int main() {
  ...
  pw::system::Init();
  auto& rpc_server = pw::system::GetRpcServer();

  board_service.Init(pw::system::GetWorkQueue(), am::system::Board());
  rpc_server.RegisterService(board_service);

  air_sensor_service.Init(am::system::AirSensor());
  rpc_server.RegisterService(air_sensor_service);
